...
const INITIAL_MESSAGE: Message = {
  id: '1',
  role: 'model',
  text: "Hoot Hoot! 🦉 I'm Owl! Do you want to find a magical story, or ask me a fun question?",
};
...
function BouncingDots() {
  ...
  return <Text style={{ fontSize: Typography.bodyChild, lineHeight: 24, color: Colors.accentSage, fontWeight: '800', letterSpacing: 2, paddingHorizontal: 4 }}>{dots || ' '}</Text>;
}

export default function OwlChildTab() {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      const latest = await getLatestOwlConversation('child', userId, activeProfileId);
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    saveOwlConversation('child', userId, activeProfileId, {
      id: conversationId,
  ...
  const sendMessage = async () => {
    ...
    xhr.onerror = () => {
      setMessages(prev => prev.map(m => m.id === messageId ? { ...m, text: "Hoot! Sorry, something got tangled in my feathers. Try asking again! 🪵" } : m));
      setLoading(false);
    };
...
const s = StyleSheet.create({
  ...
  headerTitle: { fontSize: Typography.titleChild, fontWeight: '800', color: Colors.accentSage },
  chatArea: { flex: 1 },
  bubble: { 
    maxWidth: '85%', 
    padding: Spacing.md, 
  ...
  input: {
    ...
    paddingVertical: 14,
    fontSize: Typography.bodyChild - 2,
